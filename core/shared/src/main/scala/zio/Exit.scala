...
sealed trait Exit[+E, +A] extends Product with Serializable { self =>
   ...
   */
  final def fold[Z](failed: zio.Cause[E] => Z, completed: A => Z): Z =
    self match {
   ...
   * Sequentially zips the this result with the specified result or else returns the failed `Cause[E1]`
   */
  final def foldM[R, E1, B](failed: zio.Cause[E] => ZIO[R, E1, B], completed: A => ZIO[R, E1, B]): ZIO[R, E1, B] =
    self match {
      case Failure(cause) => failed(cause)
      case Success(v)     => completed(v)
    }
   ...
   */
  final def getOrElse[A1 >: A](orElse: zio.Cause[E] => A1): A1 = self match {
  ...
  final def zipWith[E1 >: E, B, C](that: Exit[E1, B])(
    f: (A, B) => C,
    g: (zio.Cause[E], zio.Cause[E1]) => zio.Cause[E1]
  ): Exit[E1, C] =
    (self, that) match {
